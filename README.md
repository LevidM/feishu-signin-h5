...
| `VERIFY_CACHED_RECORDS` | 否 | `true` | 缓存命中后是否向飞书精确校验记录仍存在；建议生产保持开启 |
...
生产环境建议始终带上 `table` 参数，尤其是同一个多维表格里有多张活动表时。自动查找适合本地调试或单表场景，现场签到应使用插件生成的完整链接：

```text
https://signin.example.com/?app={bitable_token}&table={table_id}
```
...
- 表格字段配置和手机号索引都按 `bitable_token + table_id` 隔离，生产链接建议始终带 `table` 参数。
- 打开签到页触发 `/api/config` 后，后端会自动后台预热手机号索引。
- `/api/config` 和 `/api/signin` 都有限流保护；配置 Redis 后限流计数在多个 worker 间共享。
- 活动开始前可以主动调用 `/api/cache/preload` 预热，降低首个签到请求耗时。
- 签到时如果手机号命中缓存，直接使用缓存，不再定时每 10 分钟重新拉取。
- `VERIFY_CACHED_RECORDS=true` 时，后端会在返回“已签到”、展示同手机号多候选、或无字段可写但准备返回成功前，向飞书做一次精确校验；如果报名记录已从多维表格删除，会同步清理 Redis/内存索引并返回未报名。
- 签到时如果手机号未命中缓存，会先通过飞书 `records/search` 按手机号精确补查，并把查到的新报名记录局部合并进缓存。
- 签到请求内不会执行全量拉取；缓存为空或精确补查失败时，会启动后台预热并让用户稍后重试，避免现场请求被全表同步拖慢。
- 预热和精确补查会尽量只读取手机号、姓名、座位、签到状态、签到时间等必要字段，减少飞书响应体大小。
- 飞书 API 出现短暂 `429` 或 `5xx` 时会按 `FEISHU_API_MAX_RETRIES` 和 `FEISHU_API_RETRY_BASE_DELAY` 做有限重试。
- 后端复用飞书 HTTP 连接池，减少每次签到重新建连的耗时；连接数由 `FEISHU_HTTP_MAX_CONNECTIONS` 和 `FEISHU_HTTP_MAX_KEEPALIVE_CONNECTIONS` 控制。
- 同一个 `bitable_token + table_id + phone` 会加锁处理，配置 Redis 后该锁跨 gunicorn worker 生效。
...
- 活动开始前 5-15 分钟调用 `/api/cache/preload`，让大多数签到走「缓存命中 + 飞书写入」路径。
- 生产必须配置 Redis；多 worker 下才能共享缓存、锁和限流计数。
- 签到链接必须带 `table` 参数，避免现场请求触发自动识别表格。
- 正常缓存命中时，后端只需要一次飞书更新请求；如果飞书 Open API 本身响应慢或限流，无法保证 2 秒内成功返回，只能快速失败并提示重试。
- 如果现场更关注极限性能，可临时设置 `VERIFY_CACHED_RECORDS=false`，减少重复签到和多候选场景的额外精确查询；代价是多维表格中已删除的报名记录可能在 Redis 过期前仍被缓存命中。
- 大型活动建议提前用少量手机号压测签到接口日志里的 `elapsed_ms`，确认现场网络到飞书 Open API 的实际延迟。
